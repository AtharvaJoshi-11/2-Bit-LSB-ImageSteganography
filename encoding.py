...
'''EVERY CHARACTER NEEDS ONE BYTE (8 BITS). HERE WE STORE INFO INSIDE RED AND GREEN PIXELS ONLY.
   HENCE EACH CHARACTER NEEDS 2 PIXELS.
   USING SUBSTITUTION CIPHER WITH TRANSPOSITION CIPHER TOGETHER TO CREATE A HIGHER LEVEL OF ENCRYPTION'''
...
# Applying encryption(caesar + railfence)
input_text = caesar_cipher.cipher(input_text)
input_text = railfence_cipher.encryptRailFence(input_text, 5)
...
reconstructed_image.save('img-1(r).png')
print("Reconstructed image:'img-1(r).png'",'created')
